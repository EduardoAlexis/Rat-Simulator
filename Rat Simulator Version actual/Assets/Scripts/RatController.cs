...
public class RatController : MonoBehaviour {
    ...
    void Update () {
        ...
        if (canMove)                                              //Controlador de movimiento , dentro de un if, para que se pueda mover o no
        {
            ...
            if (Izquierdabot.ispressed || Input.GetKey(KeyCode.A)) // Si se presiona el boton o la tecla A
            {
                Izquierda(); Miratica.SetBool("derecha", true); Miratica.SetBool("izquierda", false); //  se desactiva izquierda y se activa derecha
            }else
            {
                Miratica.SetBool("derecha", false); // sino se desactiva derecha
            }
            if (Derechabot.ispressed || Input.GetKey(KeyCode.D))  // Si se presiona el boton o la tecla D
            {
                Derecha(); Miratica.SetBool("izquierda", true); Miratica.SetBool("derecha", false); //  se desactiva derecha y se activa izquierda
            }
            else
            {
                Miratica.SetBool("izquierda", false); // sino se desactiva izquierda
            }
    ...
    {
        transform.Translate(Vector3.right * -Time.deltaTime * Speed);   // Movimiento
    }
    public void Abajo()
    {
        transform.Translate(Vector3.right * Time.deltaTime * Speed);  // Retroceso
    }
